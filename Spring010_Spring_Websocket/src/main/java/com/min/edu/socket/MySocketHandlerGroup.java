...
public class MySocketHandlerGroup extends TextWebSocketHandler{
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		String txt = "";
		logger.info("handleTextMessage() 전송된 메세지 : {}",msg);
		
		Map<String, Object> mySession = session.getAttributes(); //handshake-interceptors에 의해서 WebSocketSession 값을 HttpSession 값으로 변경
		...
		if(msg.indexOf("#&nick_")!=-1) {
			for(WebSocketSession s : list) {
				...
				if(grSession.equals(otherGrSession)) { //같은 그룹의 세션에게 입장 메세지를 전달
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		//접속을 종료하고자 하는 WebSocketSession의 정보가 있는지 확인
		...
		for (WebSocketSession s : list) {
			...
			if(myGrSession.equals(otherGrSession)) {
				s.sendMessage(new TextMessage("<font color='blue' size='2px'>"+myMemSession+"님이 퇴장하셨습니다("+now+")</font>"));
			}
