...
public class MySocketHandlerOne_To_Many extends TextWebSocketHandler{
	...
	private Map<WebSocketSession, String> map = new HashMap<WebSocketSession, String>(); //WebSocketSession에 매핑된 이름(닉네임)
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		logger.info("방금 참여한 WebSocketSession 정보 : {}".concat(session.getId()));
		
		super.afterConnectionEstablished(session);
		
		list.add(session); //현재 참여한 WebSocketSession을 목록에 담아준다
		logger.info("현재 참여하고 있는 WebSocketSession의 개수 {}", list.size());
		Map<String, Object> map = session.getAttributes();
		logger.info("------------ session.getAttributes() ------------\n {}",map);
	}

	//Client 화면에서 WebSocket 객체가 닫혔을 때 => ws.onclose()
	@Override
	...
	//Client 화면에서 Message를 전달받아 모든 참여 WebSocketSession에 Broadcasting을 해줌 => ws.send
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		logger.info("전달된 메세지 getPayload : {}",msg);
		logger.info("전달된 메세지 toString : {}",msgToString);
		...
		if(msg != null && !msg.equals("")) {
			...
			if(msg.indexOf("#&nick_") != -1) {
				...
				} else { // #&nick_ 없다면 채팅글이기 때문에 모든 참여자에게 메세지를 보냄
					for (WebSocketSession s : list) {
